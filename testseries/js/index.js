...
function doLogin(){
  ...
  var em = document.getElementById("inputEmail").value+'@healthofficer.com';
...
function getExamPaperById(examId) {
  ...
  console.log(examId);
    ...
    if (examId=='demo') {
      ...
      var docRef = db.collection("healthofficer").doc(examId);
  ...
  setTemplate('viewTemplate','oldexamPage',function() {
    ...
    console.log(globalData.oldExamJson);
      if (globalData.oldExamJson) {
        ...
        var docRef = db.collection("healthofficer").doc("oldExamList");
        docRef.get().then(function(doc) {
          if (doc.exists) {
